...
const eglRxceptionRouter = express.Router();
...
// })
eglRxceptionRouter.get('/get', async (req, res) => {
    try {
        const result = await getPool().request().query('SELECT * FROM T_GL_INBOUND_REPORT');
...
eglRxceptionRouter.get('/get/:systemId', async (req, res) => {
    try {
        const systemId = req.params.systemId
        //console.log(systemId, "systemId")
        const result = await getPool().request().query(`SELECT * FROM T_GL_INBOUND_REPORT WHERE APPLN_CODE = '${systemId}'`);
...
eglRxceptionRouter.get('/unmapped/:systemId', async (req, res) => {
    try {
        const systemId = req.params.systemId
        //console.log(systemId, "systemId")
        const result = await getPool().request().query(`SELECT * FROM T_EGL_HO_OUTBOUND_${systemId}_DTL WHERE SUSP_AC_6_NO = 'true' OR SUSP_UNIT_NO = 'true' OR SUSP_CO_CODE = 'true'`);
        // const uniqueArray = result.recordset.reduce((acc, current) => {
        //     if (!acc.some((item) => item.EFF_DTE === current.EFF_DTE)) {
        //       acc.push(current);
        //     }
...
eglRxceptionRouter.get('/suspended/:systemId', async (req, res) => {
    try {
        const systemId = req.params.systemId
        //console.log(systemId, "systemId")
        const result = await getPool().request().query(`SELECT * FROM T_EGL_HO_OUTBOUND_${systemId}_DTL WHERE SUSP_ENABLED = 'Y'`);
        // const uniqueArray = result.recordset.reduce((acc, current) => {
        //     if (!acc.some((item) => item.EFF_DTE === current.EFF_DTE)) {
        //       acc.push(current);
        //     }
...
//     }
// });
module.exports = eglRxceptionRouter ;